...
// Área Administrativa - Padaria Paula
// Sistema completo de gestão (acesso com PIN)
...
export default function AdminPage() {
  ...
  // Verificar se é tela de PDV
  const isTelaPDV = telaAtual === 'produtos';
      ...
      case 'produtos':
        return (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 h-full">
            <div className={`lg:col-span-2 ${itens.length > 0 ? 'pb-24 lg:pb-0' : ''}`}>
              <CategoriasPDV />
            </div>
  ...
  return (
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto container mx-auto px-2 sm:px-4 py-3 sm:py-4 max-w-7xl">
        {renderConteudo()}
      </main>

      {/* Carrinho fixo no mobile quando na tela de produtos */}
      {isTelaPDV && itens.length > 0 && (
