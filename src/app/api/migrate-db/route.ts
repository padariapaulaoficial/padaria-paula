...
export async function GET() {
  ...
  try {
    ...
    const pedidoColumns = [
      ...
      { name: 'statusPagamento', sql: `ALTER TABLE "Pedido" ADD COLUMN IF NOT EXISTS "statusPagamento" TEXT DEFAULT 'PENDENTE'` },
    ];
    ...
    // Verificar colunas faltantes na tabela Orcamento
    const orcamentoColumns = [
      { name: 'valorTeleEntrega', sql: `ALTER TABLE "Orcamento" ADD COLUMN IF NOT EXISTS "valorTeleEntrega" DOUBLE PRECISION` },
    ];

    for (const col of orcamentoColumns) {
      try {
        await db.$executeRawUnsafe(col.sql);
        results.push(`Coluna Orcamento.${col.name} verificada/adicionada`);
      } catch (err: any) {
        results.push(`Coluna Orcamento.${col.name}: ${err.message}`);
      }
    }
